...
import { KVNamespaceListKey } from '@cloudflare/workers-types';
...
async function getRecentNews() {
  ...
  if (list.ok) {
    // TODO: type as KVNamespaceListKey[]
    const infoData: KVNamespaceListKey<unknown, string>[] = await list.json();
    return infoData;
  }
...
export default function RecentNews() {
  ...
  useEffect(() => {
    getRecentNews()
      .then(data => {
        if (data) {
          setNewsList(data.map((key: any) => key.name));
        }
      })
      .catch(err => {
